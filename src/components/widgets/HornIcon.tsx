...
const AudioPlayer = (props: HornIconProps, ref) => {
    const { className, style, phonetic, size = 'base', word, autoPlay } = props;
    ...
    return (
        <label>
            {phonetic && <span className="mr-5">/{phonetic}/</span>}
            <button
                type="button"
                onClick={() => audioRef.current.play()}
                className={`__HornIcon text-${size} focus:outline-none dark:fill-gray-400 dark:opacity-80 cursor-pointer text-gray-600 align-middle ${className}`}>
                <svg
                    className={className}
                    {...SIZE_MAP[size]}
                    style={{ ...style }}
